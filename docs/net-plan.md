...
> **Status: implementation in progress.** The source-level
...
| WebSocket | Partial | Verified `ws`/`wss` client handshake, subprotocols, masked sends, fragmented high-level receive, UTF-8/close/limit checks, automatic pong, server-side `Net.HTTP.Server` upgrade routes with subprotocol negotiation and unmasked server framing | Raw frames, heartbeat, reconnecting client, scripted mock, browser implementation |
