...
> **Implementation status:** the first usable Tey bootstrap is in `tey/`.
> Known compiler and package-manager gaps are tracked in
> [`tey-limitations.md`](tey-limitations.md).
...
|---|---|---|
| Runtime | **BEAM (Core Erlang)** | Kex lowers through IR to Core Erlang (`src/ir/`) and runs on BEAM. The tree-walk interpreter is a development fallback, not a target. Tey runs on BEAM. |
...
| Manifest | **`package.kex`** (declarative Kex DSL) | Uses familiar Kex syntax, but is parsed as data and never evaluated. |
...
- Tey's entry point lives at `tey/src/main.kex`; all implementation source lives under
  `tey/src/tey/*.kex` and declares a `Tey.*` module (`semver.kex` →
  `module Tey.Semver`, …). The language allows several modules per file and
...
`src/ir/emit_core.cxx` emits Core Erlang (`.core`), and `kex --emit-core`
...
Idiomatic, declarative Kex DSL. Tey parses only the manifest declarations shown
below; it does not inject a prelude, evaluate the file, start a subprocess, or
...
tey/
  src/
    ...
    tey/
      ...
      git.kex         # foul: wraps Process.run("git", …)
      manifest.kex    # declaratively parse package.kex
      lockfile.kex    # read/write tey.lock; hash verification
      cache.kex       # global content-pinned source cache
      resolver.kex    # dependency resolution
      toolchain.kex   # compiler/runtime/stdlib installation and selection
      commands.kex    # one function per command
      cli.kex         # OptionParser configuration and dispatch
