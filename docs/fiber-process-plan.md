...
- `make spec-beam` — every spec through `-R` (BEAM), diffed against the exact same `.expected` golden files the tree-walker satisfies. **Informational only** (always exits 0) — currently 77/104 matching. Not gated on, because the remaining 27 exercise real, separately-scoped feature gaps in the codegen backend, not regressions — see below.

### Real bugs found and fixed along the way

Chasing the BEAM parity gap from 16/104 to 77/104 surfaced a long tail of genuine, previously-latent bugs in the codegen backend (`src/codegen/core_erlang.cxx`) and its Erlang runtime (`runtime/src/*.erl`), independent of anything specific to wasm/fibers above:
...
### Two genuine, unresolvable-by-value representational ambiguities
...
- **Currying/function references** (`~(+)`, `&alpha?`) — zero codegen support; `CurryExpr`/`CurryPlaceholder` AST nodes aren't handled at all.
