...
### JS REPL bindings (`src/wasm_repl.cxx`) — working test page, yield-crossing bug resolved

`src/wasm_repl.cxx` is a small C API (`kex_repl_create`/`kex_repl_eval`/`kex_repl_last_result`/`kex_repl_complete`/`kex_repl_destroy`) wrapping one persistent `Evaluator` + `SemanticDB` per session — same cross-call state persistence already proven in phase 1 (a `let`/`spawn` on one call stays visible/alive on the next). Built as its own wasm target (`kex_repl_wasm`, `-sMODULARIZE` + `EXPORTED_FUNCTIONS` + `--preload-file src/prelude@/prelude`), deliberately not reusing `main.cxx`'s interactive REPL loop (built around a real blocking stdin, which doesn't exist in a browser).

**`web/index.html`** is a self-contained test page built on a **real terminal emulator** (xterm.js, vendored under `web/vendor/xterm/`), with a from-scratch readline-equivalent line editor in JS (cursor movement, Backspace, Ctrl-A/E/K/U/W, Ctrl-C, Up/Down history persisted via `localStorage`, multi-line `do...end` continuation) and TAB completion. Everything is a straight port of, or a direct call into, the exact same logic `main.cxx`'s real readline-based REPL uses:
- `countBlocks()` (multi-line continuation), `looksLikeTopLevelDef()` (def-vs-expression classification) — ported 1:1 from `main.cxx`.
- Tab completion doesn't re-implement anything: `kex_repl_complete` calls the same header-only `kex::resolveCompletionQuery`/`kex::rewriteCompletions` (`src/common/completion.hxx`) against the same `SemanticDB`, seeded with the same prelude (`kex::loadPrelude`, factored out of `main.cxx` into `src/common/prelude_loader.hxx` so both the native REPL and this one call the identical function — native reads `KEX_PRELUDE_DIR` off the real filesystem, wasm reads `/prelude`, a path `--preload-file` embeds into MEMFS at build time). JS only has to replicate the *word-boundary scan* readline itself does (`WORD_BREAK_CHARS`, matching `rl_completer_word_break_characters` exactly) to find `start`/`text`, then hands those straight to the wasm export.
- ANSI coloring: `kex::color::enabled` was left on (previously forced off, back when the page was a plain-text textarea) and `kex_repl_eval`'s result/error formatting was rewritten to match `main.cxx`'s `showResult`/`catch` blocks byte-for-byte — the wasm binary emits the exact same escape codes the native REPL does, and xterm.js renders them, so no color logic is duplicated in JS at all.

Verified end-to-end with real keystrokes driven through Playwright/Chromium: unique tab completion (`IO.printL<TAB>` → `IO.printLine`), completion of a user-defined function immediately after its `let`, ambiguous completion showing a match list (`[1,2,3].<TAB>` → the full stripped `List` member list, matching `kexDisplayMatches`' shared-qualifier-prefix stripping), history recall, in-place line editing, Ctrl-C cancel, and a multi-line `do...end` block.

Two more real, separate bugs turned up building this and were fixed the same way as everything else in this file — by actually loading the page in a browser, not just smoke-testing in Node:
- `SemanticDB`'s `FileState` member needs the complete `Diagnostic` type to be destructible; `semantic/db.hxx` only forward-declares it. `wasm_repl.cxx` didn't hit this before since it never touched `SemanticDB` — adding tab completion did, requiring `#include "semantic/analyzer.hxx"` (where `Diagnostic` is fully defined) even though nothing in the file names `Diagnostic` directly.
- Emscripten's `--preload-file` package (`kex_repl_wasm.data`) resolves relative to `window.location.pathname` (the *page's* URL) by default, not the JS module's own directory — 404'd at `/web/kex_repl_wasm.data` instead of `/build-wasm/kex_repl_wasm.data`. Fixed by passing an explicit `locateFile` to `createKexReplModule({ locateFile: (path) => "../build-wasm/" + path })`.
...
**The yield-crossing bug is fixed.** Root cause: this is a real, documented Emscripten bug — [emscripten-core/emscripten#13302](https://github.com/emscripten-core/emscripten/issues/13302), "Bad return value from WASM function with Asyncify and fibers." Any `ccall(..., {async: true})`'d export that performs a `emscripten_fiber_swap` (exactly what happens here whenever `receive`/`receive timeout:`/`Task.await` actually blocks and the scheduler switches processes) returns garbage to JS and can leave the call effectively replayed, *unless* something Asyncify itself recognizes as an async operation (like `emscripten_sleep`) also runs before the function returns. The two symptoms chased earlier — a direct return value not reaching JS at all, and yield-crossing calls showing duplicated output/lost state — are both this same underlying issue; the first was worked around structurally (splitting `kex_repl_eval` into a void call + separate `kex_repl_last_result` fetch, still in place), and the second is fixed by the issue's own documented workaround: `wasm_repl.cxx`'s `kex_repl_eval` now calls `emscripten_sleep(0)` right before returning. `emscripten_sleep(0)` still defers one tick to the browser event loop (imperceptible at 0ms) but "kicks" Asyncify's return-value/rewind plumbing back onto the right track.

Verified with a full `spawn`/`send`/`receive`/`receive timeout:`/`Task.start`/`.await` workflow — everything inside one `spawn do...end` block so process identity stays consistent across the whole scenario within a single `kex_repl_eval` call — run twice in a row through real Playwright/Chromium keystrokes: both runs produced identical, correct, non-duplicated output (`task ok 30` from `Task.await`), with no state bleeding between runs. Native and wasm test suites both still 145/145 after the fix.

## Cross-backend parity: BEAM vs. tree-walker vs. wasm

There are now three ways to run a Kex program — the tree-walking interpreter (native and wasm), and the BEAM/Core Erlang codegen backend — and CI checks all three against the spec suite:

- `make spec` — the tree-walker, native. Gates CI (106/106).
- `make spec-wasm` — the tree-walker, wasm-built `kex` CLI via Node (`-sNODERAWFS=1`, giving it real host filesystem access — see `CMakeLists.txt`'s `kex` target — deliberately not applied to `kex_repl_wasm`, which needs to run in an actual browser where NODERAWFS doesn't apply). Gates CI (104/104 — same tree-walker as native, so a mismatch here would be a genuine wasm-specific regression).
- `make spec-beam` — every spec through `-R` (BEAM), diffed against the exact same `.expected` golden files the tree-walker satisfies. **Informational only** (always exits 0) — currently 80/104 matching. Not gated on, because the remaining 24 exercise real, separately-scoped feature gaps in the codegen backend, not regressions — see below.

### Real bugs found and fixed along the way

Chasing the BEAM parity gap from 16/104 to 80/104 surfaced a long tail of genuine, previously-latent bugs in the codegen backend (`src/codegen/core_erlang.cxx`) and its Erlang runtime (`runtime/src/*.erl`), independent of anything specific to wasm/fibers above:

- **`if`/`break`/`next` inside a `loop`/`while` body could infinite-loop or silently drop code.** `emitLoopBodyFrom`'s recursive helpers didn't thread through "what happens when this sub-body is exhausted" — an `if`'s own then/else branch falling off the end was wrongly treated as "restart the loop" instead of "continue with whatever comes after the `if`". Fixed by threading an explicit `fallthrough` parameter through every recursive call. The same gap existed for `break`/`next` inside a bare `TrailingIf` (`break if cond`) and inside `match` arms (`5 -> break`), and for a `loop`/`while` nested inside another loop's body (which had no dispatch case in `emitLoopBodyFrom` at all, hitting codegen's generic "unimplemented AST node" fallback).
- **`&&`/`||` didn't short-circuit.** Compiled to Erlang's strict `'and'`/`'or'` BIFs, which evaluate both operands unconditionally — unlike source-level `andalso`/`orelse`. Caused genuine infinite recursion for `n == 0 || isOdd(n - 1)`-shaped mutual recursion (every call is a real tail call, so it never stack-overflows either — it just runs forever). Fixed by expanding to the same case-based short-circuit form the real Erlang compiler generates for `andalso`/`orelse`.
- **`/` didn't match Kex's int-vs-float division semantics.** Erlang's `/` always returns a float; Kex's `/` does integer division (truncating, like Erlang's `div`) when both operands are integers. Fixed with a runtime `is_integer` check dispatching to `div` or `/`.
- **A char-literal pattern (e.g. `@['_' | rest]`) compiled to Erlang's wildcard `_`.** `LiteralPattern`'s emission had no case for `TokenType::Char`, falling to the `default: return "_"` branch — meaning the pattern matched *any* character, not just that one, silently making later, more specific clauses unreachable dead code.
- **A single-clause function whose only parameter was a non-trivial pattern (e.g. `let head(@[x | _]) = x`) produced no binding for the pattern's variables at all.** The "single clause, skip the case-dispatch" fast path assumed `bindPatternLets` could turn any pattern into plain `let`s, but `bindPatternLets` only ever supported `VarPattern`/`RecordPattern`. Fixed by falling through to the general (fully pattern-matching-capable) case-dispatch path whenever a clause's parameters aren't all "simple".
- **The receiver-pattern-detection check (`make Type do let method(@pattern) ... end`) only recognized `RecordPattern`, not the `@` sigil itself.** `@` always wraps its inner pattern in `ThisPattern` regardless of the inner kind (`Parser::parsePattern`) — checking for `RecordPattern` directly missed list-cons receivers (`@[x|rest]`) entirely, and (once broadened too far, fixing that) briefly *over*-matched a bare type-name parameter like `to(String)` used for `.to(Type)` dispatch (not `@`-prefixed, a `ConstructorPattern`, not a receiver) as if it were the receiver, leaving the real implicit `this` unbound. The precise fix checks for `ThisPattern` (the actual `@` marker) or bare `RecordPattern` specifically.
- **`erlc`'s own warnings leaked into the running program's stdout.** The `-R` compile step merged erlc's stderr into stdout (`2>&1`) specifically to "suppress" it per the code's own comment — the exact opposite of what it did. erlc also prints warnings to its *own* stdout, not just stderr, so both streams needed silencing (`> /dev/null 2>&1`), not just one.
- **An explicit `main do...end` was required for `-R`/`--compile`, even though the codegen already synthesizes one from trailing bare top-level expressions when there isn't one.** A leftover, overly strict guard in `main.cxx` rejected valid programs the emitter could already handle.
- **`erlang:negate/1` isn't a real BIF** (unary minus is `erlang:'-'/1`) — undef'd on any negative-number expression.
- Several **stdlib gaps**: `ENV` (compiled as if it were a module namespace instead of the real `Map<String,String>` value it is), `Integer.parse`/`Float.parse` (need `Ok`/`Error`-wrapping, not a bare BIF forward), `Math.E`/`.log10`/`.hypot`/`.cbrt`/`.log(x, base)` (Erlang's `math` module doesn't have several of these, or doesn't support 0-arg/2-arg forms Kex needs), `assert` (part of the testing DSL, a plain global function, not stdlib-namespaced — fell through to "unbound variable"), `File.append`, list `.first`/`.last`/`.min`/`.max` (need `Just`/`None`-wrapping, not the raw — and crashing on empty — element/BIF), `.get`/`list[i]`/`.at(i)` (unconditionally assumed a `Map` receiver, `{badmap,...}` for any list), `.indexOf`/`.partition`/`.flatMap`/`.product`, `.or(default)` (only handled `Just`/`None`, not `Ok`/`Error`), `.in?(range)`, char predicates (`digit?`/`alpha?`/`space?`, plain global functions like `assert`).
- **Display formatting**: floats used Erlang's `~g` (variable significant digits — `"3.14159"` instead of `"3.141593"`) instead of matching the tree-walker's actual algorithm (fixed decimal-6 then trim trailing zeros); `None` printed as lowercase `none`; tuples used Erlang's `{a,b}` syntax instead of Kex's `(a, b)`; nested strings inside lists/tuples were quoted (`["abcd"]`) when the tree-walker's `toString()` convention never quotes at any nesting depth (`[abcd]`).
- **Currying/function references** (`~add(1)`, `~(+)`, `&alpha?`) — `CurryExpr`/`CurryPlaceholder`/`ShorthandLambda`'s `Kind::Function` case had zero or wrong codegen. Two real bugs here specifically: (1) a partial application with *no explicit placeholder* but fewer bound args than the target's arity (`~add(1)` on a 2-arity `add`) needs *implicit* trailing parameters appended after the bound ones — only building one Erlang parameter per explicit `_` placeholder produced a 0-arity fun for this shape, an arity mismatch the moment it was called; (2) `&alpha?`/`&digit?`/`&space?` reference plain global builtins that weren't real top-level Erlang functions in the compiled module at all (they were inlined at the direct-call site) — `apply 'alpha?'/1(...)` undefs unless the builtin is *also* exposed as a real `kex_io` function the reference can resolve to.

### Three genuine, unresolvable-by-value representational ambiguities

A few mismatches can't be fixed with a value-based heuristic, because two distinct Kex concepts compile to the *exact same* Erlang runtime representation, with nothing left at runtime to tell them apart:

- **Char vs. Integer** — both plain Erlang integers. `x.upperCase + rest` (Char + String) can't be distinguished from real integer arithmetic by value; `kex_io:add/2` biases toward "integer mixed with a list is a char code to splice in" (Kex never legitimately adds an integer to a list otherwise, so this has no real downside). This bias doesn't cover every case, though: `'a' + 'b'` (Char + Char — *neither* operand is a list) falls through to plain integer arithmetic (`195`, i.e. `97 + 98`) instead of concatenating to `"ab"`, and `"hello".at(1)` (returns a Char) displays as the bare integer `101` instead of `e` — both need Chars to carry their own runtime tag to fix properly, not just a smarter heuristic on `+`.
- **Empty list vs. empty string**, and **a plain structural `Tuple` vs. an ADT variant's payload** (`(Underscore, rest)` vs. `Just(1)` — both just `{Atom, ...}`) — for these, a value-based heuristic can satisfy some specs at the direct expense of others (tried making empty lists print `"[]"`, which fixed `env.kex` but broke `my_capitalize.kex`'s empty-string result). Current compromise: empty lists render blank (matching the more common case), and only the reserved prelude ADT tags (`Just`/`Ok`/`Error`/`Some`) get `Tag(args)` rendering — anything else with an atom-like first tuple element falls back to plain `(a, b, ...)` tuple syntax. Neither generalizes to arbitrary user-defined types in the same ambiguous shape.
- **A mapped String vs. a mapped List, inside `.flatMap`** — the same list/string conflation as above, one level removed: `.flatMap { block }`'s "splice if the block's result is itself a list" (matching the tree-walker's `ListValue`-specific check) can't distinguish a block returning a `String` (should be pushed as one element) from one returning a real `List` (should be spliced) — `["hello", "world"].flatMap { |s| s }`-shaped code splices character-by-character into `"helloworld"` instead of producing `[hello, world]`.

### Remaining known gaps — whole missing features, not bugs

Each of these is a real, separately-scoped codegen feature that doesn't exist at all yet, not a quick fix:

- **Cross-type method-name collisions.** Two different `make Type do ... end` blocks defining the same method name (e.g. both `Vec2` and `Vec3` defining `add`) currently emit two Erlang functions with the same name/arity in one flat module namespace — `erlc` rejects the duplicate outright. Needs merging same-name methods across *all* make-blocks in a program into one multi-clause function that dispatches on the receiver's type tag (the same underlying problem as the next item).
- **Operator overloading** (`make Type do let +(other) ... end`) — no dispatch mechanism exists for compiling a custom operator method and routing `a + b` to it based on `a`'s runtime type tag.
- **Trait default methods** (`trait X do ... let default = ... end`, inherited by `make Type implement: X do ... end` unless overridden) — no trait-awareness in codegen at all.
- **Streams/lazy evaluation** — crashes with a `function_clause` error; the Stream runtime representation codegen assumes doesn't match what's actually produced.

As of this writing: 80/104 specs match (`make spec-beam`).

Run `make spec-beam` for the current exact list of differing specs.


The repros that had ruled out simpler explanations earlier (plain `sleep_until` in an async ccall; a bare `emscripten_fiber_swap` + `sleep_until` in one call, with no `Scheduler`/`Evaluator` involved) make sense in hindsight: they likely didn't reproduce the bug because they were too simple to trigger more than one fiber swap per call, or because the isolated repro's own structure happened to avoid the exact unwind/rewind path the real `Scheduler` hits when a process actually blocks and control returns to a *different* still-running process rather than straight back to the caller.
