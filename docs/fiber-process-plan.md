...
**Not yet attempted at all**: real filesystem access for the wasm CLI/REPL (preloading `examples/`/`src/prelude/` into MEMFS, or `NODERAWFS` for a Node-hosted CLI use case specifically), JS bindings exposing `Scheduler::step()` to a browser event loop, and any actual browser UI. A minimal test HTML page for trying the in-browser REPL is the planned next step.
