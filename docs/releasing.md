...
| VERSION        | GitHub release | Image tags              | Homebrew  |
| -------------- | -------------- | ----------------------- | --------- |
| `0.4.0`        | stable         | `0.4.0`, `latest`       | updated   |
| `0.4.0-rc.1`   | pre-release    | `0.4.0-rc.1`, `rc`      | untouched |
| `0.4.0-beta.2` | pre-release    | `0.4.0-beta.2`, `beta`  | untouched |
...
4. **`manifest`** — stitches each variant's two digests into one multi-arch
...
5. **`publish`** — compiles the release log, then **creates the tag** (via
...
rewrite commits nothing when it changes nothing.
