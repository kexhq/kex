...
int main() {
    ...
    describe("Completion pipeline — prelude", []() {
        ...
        it("offers a module's own members, not its types' fields", []() {
            auto db = makePreludeDb();
            // `Kex` declares the constant VERSION and the record Version.
            // The record's FIELDS and its make-block methods are members of
            // Version, not of Kex — they carry the declaring module only
            // incidentally, and used to leak here.
            auto members = simulate(db, "Kex.", 0, "Kex.");
            assertTrue(has(members, "Kex.VERSION"), "VERSION missing");
            for (const auto& leaked : {"Kex.major", "Kex.minor",
                                       "Kex.patch", "Kex.revision",
                                       "Kex.number", "Kex.tuple"})
